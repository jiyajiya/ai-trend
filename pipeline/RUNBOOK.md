...
### 상세 분석(`data/analysis/<id>.json`)

상세 분석은 `summarized.json`에 넣지 말고 **항목별 파일**로 즉시 저장한다
(`data/analysis/<항목 id>.json`, 내용은 아래 객체 하나). 한 항목이 실패해도 나머지가 살아남고,
`feed.json`이 가벼워지며 매일 커밋되는 diff가 신규 파일 추가로 끝난다.
merge가 파일 존재를 보고 `feed.json` 항목에 `hasAnalysis: true`를 붙이고, 웹 우측 패널은
카드 클릭 시 그 파일만 받아 렌더한다.

대상은 **영상·뉴스·블로그**만(SNS·repo·model 제외).
```json
{ "points": ["..."], "sections": [{ "heading": "...", "body": "..." }], "quotes": ["..."] }
```
- `points`: 핵심 포인트 3~6개(한국어, 각 한 문장).
- `sections`: 챕터/주제별 정리 2~5개. 각 `{ "heading": "...", "body": "2~4문장" }`.
- `quotes`: 주목할 인용·수치 0~3개(없으면 빈 배열 또는 생략).
- 생성 불가/대상 아님이면 파일을 만들지 않는다.
- 파일 분리 전 데이터(인라인 `analysis` 필드)도 merge가 자동으로 파일로 옮긴다.

소스 타입별 요약 입력:
- `news` / `paper`: `rawText`(요약/초록)를 읽고 요약. 부족하면 `url`을 WebFetch로 보강.
  - 본문이 충분하면 상세 분석 파일도 생성(블로그 포함). 빈약하면 생략.
...
- `youtube`:
  ...
  3) `summaryKo`를 만든 같은 트랜스크립트로 상세 분석도 만들어 `data/analysis/<id>.json`에 저장한다(fallback이면 생략).
...
launchctl print gui/$(id -u)/com.ax.ai-trend | grep state   # 등록 확인
```
해제: `launchctl bootout gui/$(id -u)/com.ax.ai-trend`
...
- git push가 비대화형으로 되려면 원격이 HTTPS + `gh auth setup-git`(자격증명 헬퍼) 상태여야 한다.
